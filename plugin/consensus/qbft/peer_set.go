...
FOR_LOOP:
	for {
		...
		case _, ok := <-pc.pongChannel:
			if ok {
				qbftlog.Debug("Send Pong")
				var pong [5]byte
				pong[0] = ttypes.PacketTypePong
				_, err := pc.bufWriter.Write(pong[:])
				if err != nil {
					qbftlog.Error("peerConn sendroutine write pong failed", "error", err)
					pc.stopForError(err)
					break FOR_LOOP
				}
			} else {
...
FOR_LOOP:
	for {
		...
		if pkt.TypeID == ttypes.PacketTypePong {
			qbftlog.Debug("Receive Pong")
		} else if pkt.TypeID == ttypes.PacketTypePing {
			qbftlog.Debug("Receive Ping")
	...
	for {
		...
		case msg := <-pc.updateStateQueue:
			...
			if typeID == ttypes.NewRoundStepID {
				...
				qbftlog.Debug("updateStateRoutine", "QbftVoteSetMaj23Msg", tmp)
