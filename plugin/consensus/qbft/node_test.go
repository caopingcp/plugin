...
func testUpdateStateRoutine(t *testing.T, pc *peerConn) {
	...
	//NewRoundStepID msg
	rsMsg := &tmtypes.QbftNewRoundStepMsg{
		Height:                int64(3),
		Round:                 int32(1),
		Step:                  int32(3),
	...
	//SetHasProposalBlock
	block := &ttypes.QbftBlock{
		QbftBlock: &tmtypes.QbftBlock{
	...
	//ProposalPOLID msg
	proposalPOL := ps.Prevotes.QbftBitArray
	proposalPOLMsg := &tmtypes.QbftProposalPOLMsg{
		Height:           int64(3),
	...
	pc.updateStateQueue <- MsgInfo{TypeID: byte(0x00)}
	assert.EqualValues(t, proposalPOL, ps.ProposalPOL.QbftBitArray)
