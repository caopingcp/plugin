...
import (
	...
	ty "github.com/33cn/plugin/plugin/dapp/qbftNode/types"
...
func main() {
	...
	case "qbftNode":
		...
		QbftNode(argsWithoutProg[1], argsWithoutProg[2], argsWithoutProg[3])
	case "perfOld":
		if len(argsWithoutProg) != 6 {
			fmt.Print(errors.New("参数错误").Error())
			return
...
func LoadHelp() {
	...
	fmt.Println("qbftNode [ip, pubkey, power]                                  : 增加/删除/修改qbft节点")
...
// QbftNode ...
func QbftNode(ip, pubkey, power string) {
	url := "http://" + ip + ":8801"
	...
	_, priv := genaddress()
	privkey := common.ToHex(priv.Bytes())
	nput := &ty.QbftNodeAction_Node{Node: &ty.QbftNode{PubKey: pubkeybyte, Power: int64(powerInt)}}
	action := &ty.QbftNodeAction{Value: nput, Ty: ty.QbftNodeActionUpdate}
